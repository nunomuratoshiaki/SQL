...
exports.lambda_handler = (event, context, callback) => {
  ...
  if (caller == 'Lambda Callback') {
    ...
    ret_sendmail = sendmail('AWSアカウント新規作成の承認依頼', DestAddr,htmlMAILBODY,htmlMAILBODY);
    // 2nd mail
    ...
    ret_sendmail = sendmail('AWSアカウント新規作成申請を受領しました',DestAddr,htmlMAILBODY,htmlMAILBODY);
    ...
    if (executionContext.State.RetryCount == 0 ) {
      ...
      ret_sendmail = sendmail('【アシスト】AWSアカウント登録用メールアドレスの有効性確認',DestAddr,htmlMAILBODY,plainMAILBODY);
    } else {
      ret_sendmail = sendmail('【再送】【アシスト】AWSアカウント登録用メールアドレスの有効性確認',DestAddr,'<p>＊＊＊お手続き未完了のお客様へ再送メールです＊＊＊</p>'+htmlMAILBODY,'＊＊＊お手続き未完了のお客様へ再送メールです＊＊＊>'+plainMAILBODY,);
    }
    ...
    ret_sendmail = sendmail('AWSアカウント登録処理が失敗しました',DestAddr,htmlMAILBODY,htmlMAILBODY);
    ...
    ret_sendmail = sendmail('【アシスト】AWSアカウント登録用メールアドレスの変更依頼',DestAddr,htmlMAILBODY,htmlMAILBODY);
      ...
      ret_sendmail = sendmail('【アシスト】AWS利用開始のお知らせ',DestAddr,htmlMAILBODY,htmlMAILBODY);   
  ...
  if (ret_sendmail == 0) {
    ...
    console.log(ret_sendmail);
    context.fail(ret_sendmail, "sendmail was failed.");
  }
};